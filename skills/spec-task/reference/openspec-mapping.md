...
| `design.md` | `plan.md` | 增加关键技术决策记录（Key Decisions：选择→替代方案→理由）+ 详细步骤分解（Steps Overview：文件路径+函数名+行为+验证） |
| `tasks.md` | `status.yaml` 中的 `steps` 字段 | 格式不同：OpenSpec 用 Markdown checkbox，spec-task 用结构化 YAML Step[] |
...
| Task Progress（checkbox 解析） | `utils/task-progress.ts` | **概念复用** — ProgressCalculator 从 status.yaml.steps 读取结构化步骤数组计算进度 |
